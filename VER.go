...
func verifySC(myCert Cert, myLedger map[*ecdsa.PublicKey]bool, revkLedger map[*ecdsa.PublicKey]bool) bool {
	_, ok := myLedger[myCert.Pubk]
	if ok {
		_, ok1 := revkLedger[myCert.Pubk]
		if ok1 {
			return false
		} else {
			return true
...
func main() {
	...
	var revokedLedger map[*ecdsa.PublicKey]bool
	issuedLedger = make(map[*ecdsa.PublicKey]bool)
	revokedLedger = make(map[*ecdsa.PublicKey]bool)
	myCert := generateCert()

	starttime := time.Now()
	for i := 0; i < 10; i++ {
		flag := verifySC(myCert, issuedLedger, revokedLedger)
		if flag == true {
