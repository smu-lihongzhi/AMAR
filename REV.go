...
func revokedSC(myCert Cert, myLedger map[*ecdsa.PublicKey]bool) bool {
...
func Invoke() {
	...
	var revokedLedger map[*ecdsa.PublicKey]bool //账本的模拟容器
	revokedLedger = make(map[*ecdsa.PublicKey]bool)
	myCert := generateCert()

	starttime := time.Now()
	for i := 0; i < 10; i++ {
		flag := revokedSC(myCert, revokedLedger)
		if flag == true {
